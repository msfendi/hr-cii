    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function ($event) {
                ...
                // ✅ UNIVERSAL SHEET (Laravel Excel + FastExcel)
                $sheet = $event->sheet;
                ...
                foreach ($this->components as $i => $component) {
                    ...
                    $active =
                        $this->groups['active_sewing'][$component->code] ?? 0;

                    $resign =
                        $this->groups['resign_sewing'][$component->code] ?? 0;
                    ...
                    if ($component->type === 'deduction') {
                        ...
                        $this->deduction['active_sewing'] += $active;
                        $this->deduction['resign_sewing'] += $resign;
                    } else {

                        $this->earning['active_sewing'] += $active;
                        $this->earning['resign_sewing'] += $resign;
                    }
                ...
                $sheet->setCellValue(
                    "B{$base}",
                    $this->earning['active_sewing']
                );

                $sheet->setCellValue(
                    "C{$base}",
                    $this->earning['resign_sewing']
                );

                $sheet->setCellValue(
                    "B" . ($base + 1),
                    $this->deduction['active_sewing']
                );

                $sheet->setCellValue(
                    "C" . ($base + 1),
                    $this->deduction['resign_sewing']
                );
