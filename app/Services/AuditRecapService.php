...
class AuditRecapService
{
     ...
     * paling dekat dengan jam mulai shift. Return null kalau tidak ketemu
     * keduanya (estimasi TS ditangani di buildRow(), bukan di sini).
     */
    protected function resolveJamPagi(string $npk, string $dateKey, array $shift, array $lateMap, array $scanMap): ?string
    {
        ...
        foreach ($scans as $scanTime) {
            $diff = abs($this->timeToMinutes($scanTime) - $shiftStartMinutes);
