...
/* Background */
#content-wrapper { background:linear-gradient(135deg,#4e73df 0%,#224abe 55%,#1a3a8f 100%) !important; min-height:100vh; }

/* Glassmorphism navbar */
.topbar { position:relative; z-index:999; background:rgba(255,255,255,.12) !important; backdrop-filter:blur(10px); border-bottom:1px solid rgba(255,255,255,.15) !important; box-shadow:none !important; }
.topbar .brand-name              { color:#fff !important; font-weight:800; font-size:.92rem; }
.topbar .nav-link                { color:rgba(255,255,255,.85) !important; font-size:.83rem; font-weight:600; padding:.3rem .65rem !important; border-radius:.4rem; }
.topbar .nav-link:hover          { color:#fff !important; background:rgba(255,255,255,.12); }
.topbar .nav-link.act            { color:#fff !important; background:rgba(255,255,255,.18); border-bottom:2px solid rgba(255,255,255,.7); }
.topbar .btn-keluar              { color:#fff !important; border-color:rgba(255,255,255,.4) !important; background:rgba(255,255,255,.1) !important; font-size:.75rem !important; }
.topbar .btn-keluar:hover        { background:rgba(255,255,255,.22) !important; }
.topbar .navbar-toggler          { border-color:rgba(255,255,255,.4) !important; padding:.2rem .4rem; }
.topbar .navbar-toggler-icon     { background-image:url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 30 30'%3e%3cpath stroke='rgba(255,255,255,0.85)' stroke-linecap='round' stroke-width='2' d='M4 7h22M4 15h22M4 23h22'/%3e%3c/svg%3e") !important; }
@media(max-width:767.98px){
    .topbar .navbar-collapse { background:rgba(26,58,143,.96); border-top:1px solid rgba(255,255,255,.12); margin-top:.25rem; padding:.45rem .5rem .55rem; border-radius:0 0 .5rem .5rem; }
    .topbar .nav-link        { padding:.38rem .25rem !important; }
    .topbar .d-flex.ml-auto  { margin-top:.4rem; padding-top:.4rem; border-top:1px solid rgba(255,255,255,.1); width:100%; justify-content:space-between; }
}

/* Heading */
.pg-title { color:#fff !important; font-weight:800; font-size:1.05rem; }
.pg-sub   { color:rgba(255,255,255,.72) !important; font-size:.78rem; }

/* ── UI Elements ── */
.main-card  { background:#fff; border:none; border-radius:.8rem; overflow:hidden; box-shadow:0 .5rem 2.5rem rgba(0,0,0,.2); margin: 10px auto; width:100%; text-align:center; }
.mc-header  { padding:.75rem 1.25rem; border-bottom:1px solid #eaecf4; background:#f8f9fc; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:.5rem; font-size:.95rem; }

/* ── Stepper ── */
.stepper-wrapper { display: flex; align-items: center; justify-content: center; margin-bottom: 2rem; }
.stepper-item { display: flex; flex-direction: column; align-items: center; flex: 1; position: relative; }
.stepper-item:not(:last-child)::after { content: ''; position: absolute; top: 18px; left: 50%; width: 100%; height: 3px; background: #e3e6f0; z-index: 0; transition: background .3s; }
.stepper-item.completed:not(:last-child)::after { background: #4e73df; }
.stepper-circle { width: 38px; height: 38px; border-radius: 50%; background: #e3e6f0; color: #fff; display: flex; align-items: center; justify-content: center; font-weight: 700; z-index: 1; transition: all .3s; }
.stepper-item.active .stepper-circle { background: #4e73df; transform: scale(1.15); box-shadow: 0 0 0 4px rgba(78,115,223,.15); }
.stepper-item.completed .stepper-circle { background: #1cc88a; }
.stepper-label { margin-top: .5rem; font-size: .8rem; font-weight: 600; color: #b7b9cc; }
.stepper-item.active .stepper-label { color: #4e73df; }
.stepper-item.completed .stepper-label { color: #1cc88a; }
...
.review-row { display: flex; justify-content: space-between; padding: .65rem 0; border-bottom: 1px solid #f0f2f8; font-size: .85rem; }
.review-row:last-child { border-bottom: none; }
.review-label { color: #858796; }
.review-value { font-weight: 700; color: #3a3b45; text-align: right; max-width: 60%; }

/* Form Field Adjustments */
.form-container { max-width: 700px; margin: 0 auto; }

/* Footer */
.sticky-footer { background:rgba(255,255,255,.08) !important; border-top:1px solid rgba(255,255,255,.12) !important; }
.sticky-footer span { color:rgba(255,255,255,.5) !important; font-size:.74rem; }
    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
            ...
            <nav class="navbar navbar-expand-md navbar-light bg-white topbar mb-4 static-top shadow">
                ...
                <button class="navbar-toggler ml-auto" type="button" data-toggle="collapse" data-target="#navbarCuti" aria-controls="navbarCuti" aria-expanded="false" aria-label="Toggle navigation">
            ...
            <div class="container-fluid px-3 px-md-4 fade-up" style="max-width: 900px; padding-bottom: 2rem;">
                ...
                <div class="mb-4 text-white">
                    <h1 class="h3 font-weight-bold mb-1"><i class="fas fa-calendar-check mr-2"></i>Pengajuan Cuti</h1>
                    <p class="mb-0" style="font-size:.9rem; opacity:0.9;">Isi form di bawah untuk mengajukan cuti Anda kepada atasan.</p>
                </div>

                {{-- Form Card --}}
                <div class="main-card text-left mb-4">
                    <div class="mc-header">
                        <span class="mc-title font-weight-bold" style="color: #3a3b45;"><i class="fas fa-file-signature text-primary mr-2"></i>Formulir Pengajuan Cuti</span>
                    </div>
                        ...
                        <form id="cuti-form" action="{{ route('pengajuan-cuti.submit-form') }}" method="POST">
                            ...
                            <div class="step-panel active" id="panel-1">
                                <div class="form-container">
                                    {{-- Karyawan Profil Box --}}
                                    <div class="alert alert-primary d-flex align-items-center mb-4 border-0" style="background:#eef2cf; border-radius:.6rem;">
                                        <i class="fas fa-id-badge fa-2x text-primary mr-3"></i>
                                        <div>
                                            <div class="font-weight-bold text-gray-900" style="font-size:.95rem;">{{ $employee->NAMA_KARYAWAN }}</div>
                                            <div class="text-muted" style="font-size:.8rem;">{{ $employee->NPK }} &middot; {{ $employee->DEPARTEMENT }}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group mb-4">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control bg-light" id="jumlah_hari_display" readonly placeholder="Otomatis dihitung...">
                                        </div>
                                        <small id="sisa_cuti_info" class="form-text mt-2 font-weight-bold" style="display:none;"></small>
                                    </div>

                                    <div class="form-group mb-4">
                                        <label class="font-weight-bold text-gray-700 small">Keterangan / Alasan <span class="text-danger">*</span></label>
                                        <textarea class="form-control" name="keterangan" id="keterangan" rows="3" required placeholder="Alasan cuti..."></textarea>
                                    </div>

                                    <div class="d-flex justify-content-between border-top pt-4 mt-2">
                                        <a href="{{ route('pengajuan-cuti.login') }}" class="btn btn-light text-gray-700 px-4">Batal</a>
                                        <button type="button" class="btn btn-primary px-4 shadow-sm" onclick="goToReview()">Review <i class="fas fa-arrow-right ml-1"></i></button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="step-panel" id="panel-2">
                                <div class="form-container">
                                    <h6 class="text-primary font-weight-bold mb-3"><i class="fas fa-clipboard-check mr-2"></i>Review Pengajuan</h6>
                                    <p class="text-muted small mb-4">Pastikan data pengajuan cuti Anda di bawah ini sudah benar sebelum melakukan konfirmasi akhir.</p>
                                    ...
                                    <div class="d-flex justify-content-between pt-2">
                                        <button type="button" class="btn btn-light px-4 text-gray-700" onclick="backToForm()">
