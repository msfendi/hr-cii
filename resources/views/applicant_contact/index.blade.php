                ...
                <div class="table-responsive">
                  <table class="table table-bordered table-sm" id="dataTable">
                    ...
                    <tbody> @foreach($applicant_contact as $contact) <tr>
                        ...
                        <!-- SEND MESSAGE -->
                        <td class="text-center"> @foreach($templates as $template) @php $alreadySent = isset($sentLogs[$contact->phone]) && $sentLogs[$contact->phone] ->where('template_id',$template->id) ->count(); @endphp @if(!$alreadySent) <a href="{{ route('applicant-contact.send',['contact_id'=>$contact->id,'template_id'=>$template->id]) }}" class="btn btn-success btn-sm mb-1">
                            {{ $template->name }}
                          </a> @endif @endforeach </td>
                        <!-- ACTION -->
