...
@push('styles')
    <style>
        ...
        .upload-zone.uploaded .uz-label {
            color: #1a6b3c;
        }

        .upload-zone .uz-hint {
            font-size: 0.65rem;
            color: rgba(67, 70, 83, .55);
            font-weight: 600;
        ...
        .doc-group-title span.material-symbols-outlined {
            font-size: 0.9rem;
        }
...
@section('content')
    ...
    <div
        ...
        <form id="step8Form" action="{{ route('recruitments.step.store', ['step' => 8]) }}" method="POST"
            ...
            <div class="px-8 py-8 rf-section-block">
                ...
                <div class="mb-7">
                        ...
                        @foreach($docs as [$field, $label, $accept, $hint, $required, $icon])
                            @if($required)
                                <div>
                                    ...
                                    <div class="upload-zone @error($field) has-error @enderror" id="zone_{{ $field }}"
                                        ...
                                        <input id="{{ $field }}" name="{{ $field }}" type="file" class="hidden"
                                            accept="{{ $accept }}" onchange="previewFile(this,'{{ $field }}')">
                                    </div>
                                    @error($field)
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 mt-4">
                        @foreach($docs as [$field, $label, $accept, $hint, $required, $icon])
                            @if(!$required)
                                <div>
                                    <div class="doc-label" style="color:#747684;">{{ $label }}
                                        @if($field !== 'scan_blanko_kesehatan')
                                            <span class="doc-required">* Wajib</span>
                                        @endif
                                    </div>
                                    <div class="upload-zone" id="zone_{{ $field }}"
                                        onclick="document.getElementById('{{ $field }}').click()">
                                        <span class="material-symbols-outlined uz-icon" id="icon_{{ $field }}">{{ $icon }}</span>
                                        <span class="uz-label" id="label_{{ $field }}">Ketuk untuk upload</span>
                                        <span class="uz-hint">{{ $hint }}</span>
                ...
                {{-- <div>
                    <div class="doc-group-title" style="background:rgba(91,94,108,.08); color:#5b5e6c;">
                        ...
                        <span style="color:#5b5e6c;">Dokumen Pendukung <span
                                style="font-weight:400;">(Opsional)</span></span>
                    </div>
            ...
            <label class="rf-declaration" for="deklarasi">
                <input type="checkbox" id="deklarasi" name="deklarasi" value="1" {{ old('deklarasi') ? 'checked' : '' }}
                    required>
                <p>
