...
<h6 class="m-0 font-weight-bold text-primary">
Data QC Insentif Master
</h6>

<div>

<a href="{{ route('qc-insentif-master.template') }}"
...
<div class="modal fade" id="importModal">
    <div class="modal-dialog">
        <form id="importForm"
              action="{{ route('qc-insentif-master.import') }}"
              method="POST"
...
let insentifTable;
    ...
    */
    ...
    /*
    | ROLE DROPDOWN (SETELAH TABLE READY)
    */
    let roleDropdown = `
        <select id="filterRole" class="form-control form-control-sm select2-role" style="width:200px">
            <option value="">Role</option>
        </select>
    `;

    $('.role-filter').html(roleDropdown);
    $('#filterRole').select2({
        placeholder: 'Role',
        allowClear: true,
        width: '200px'
...
$('#checkPeriod').on('change',function(){
    ...
    $.ajax({
        ...
        success:function(res){
            ...
            Array.from(roleSet).sort().forEach(role => {
                ...
                options += `<option value="${role}">${roleLabel}</option>`;
            });
    ...
    // AJAX MASTER TABLE
    $.ajax({
        url:'/qc-insentif-master/'+period+'/data',
        type:'GET',
        success:function(res){
