              ...
              <div class="card-body">
                <div class="table-responsive">
                  <table class="table table-bordered table-sm" id="dataTable">
                    ...
                    <tbody> @foreach($devices as $device) <tr>
                        <td>{{ $device->id }}</td>
                        <td>{{ $device->name }}</td>
                        <td>{{ $device->phone }}</td>
                        <td>
                        ...
                        <td class="text-center">
                          <div class="d-flex justify-content-center align-items-center">
                            <!-- QR BUTTON -->
                            <button class="btn btn-success btn-circle btn-sm mr-1 btn-qr" data-id="{{ $device->id }}">
                              <i class="fas fa-qrcode"></i>
                            </button>
                            <!-- DELETE BUTTON -->
                            <form action="{{ route('devices.destroy',$device->id) }}" method="POST"> @csrf @method('DELETE') <button class="btn btn-danger btn-circle btn-sm">
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-body text-center">
              ...
              <div id="qrConnected" class="alert alert-success" style="display:none"> Device Already Connected ✅ </div>
            </div>
          </div>
        </div>
      </div>
  ...
  <script>
    $('.btn-qr').click(function() {
      ...
      $.get('/devices/' + id + '/qr', function(res) {
        ...
        $('#qrImage').attr('src', 'data:image/png;base64,' + res.qr).show();
      }).fail(function(xhr) {
        $('#qrLoading').hide();
